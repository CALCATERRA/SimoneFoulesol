...
def send_view_photo_button(chat_id):
    ...
    print(f"📨 Risposta invio pulsante: {res.status_code} - {res.text}")
...
async def main(context):
    ...
    try:
        ...
        # Trigger manuale da pagina HTML
        if req.body.get("source") == "manual-return":
            chat_id = str(req.body.get("chat_id"))
            print(f"📥 Trigger manuale da pagina HTML per chat_id: {chat_id}")
            send_view_photo_button(chat_id)
            return res.json({"status": "manual_triggered"}, 200)
